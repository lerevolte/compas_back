...
## Тестовый портал avixo — полигон нового функционала

Портал **avixo** (тенант `avixo`, домен avixo.compas.pro, алиас logistopt6.compas.pro) —
тестовый. Новые фичи сначала выкатываются ТОЛЬКО туда, после проверки — переносятся
в seeds и остальные порталы.

ПРАВИЛА для нового функционала «сначала на avixo»:
1. **Никакого хардкода тенанта.** Код не должен содержать проверок вида
   `tenant('id') == 'avixo'`. Ограничение «только avixo» достигается тем, что
   метаданные/таблицы фичи установлены только в его БД, а код везде проверяет
   их наличие (`Schema::hasTable`, наличие конфига, `data_types`-записи) и
   молча выключается там, где их нет.
2. **Установка — идемпотентной artisan-командой** вида `entity:install-*` /
   `<module>:install` с аргументом `{seeds|all-tenants|<tenant_id>}` (эталон —
   `entity:install-addresses`). Одна и та же команда ставит фичу на avixo сейчас
   и на seeds/все порталы потом — без ручного SQL.
3. **Модульная структура кода.** Интеграционная логика — в Services (или в живой
   модуль, как Modules/Bitrix24), включение — по данным тенанта. Не размазывать
   фичу по ядру: точки входа (роуты, hooks, schedule) должны быть тонкими и
   guard-иться проверкой готовности фичи в текущем тенанте.
4. **Схема БД фичи** на этапе avixo создаётся install-командой
   (`CREATE TABLE IF NOT EXISTS`), а при переносе на все порталы — оформляется
   тенантской миграцией в `database/migrations/tenant/` (для новых порталов).
5. До одобрения фичи заказчиком: новые сущности ставим с `data_types.hidden=1`
   (нет в /roles, списках сущностей и аналитике, но работает по прямому URL
   `/objects/<slug>`), без `sidebar_items` (нет в меню); новые поля существующих
   сущностей прячем (`section_id` на несуществующую секцию или `hide=1`).
6. **Фичи = будущие модули со страницей управления.** В скором времени появится
   страница «Модули» для порталов, где пользователь сам ставит/удаляет нужные
   модули. Поэтому каждая фича-модуль обязана иметь ПАРУ идемпотентных команд:
   `entity:install-<x>` И `entity:uninstall-<x>` (одинаковая сигнатура target).
   Требования к uninstall:
   - по умолчанию удаляет только метаданные (data_types, data_rows,
     field_sections, settings, sidebar_items, permissions) — бизнес-данные
     (таблицы) сохраняются, чтобы повторная установка их вернула;
   - флаг `--purge` удаляет и данные (DROP таблиц, пивотов, историй);
   - патчи чужих сущностей (добавленные поля/колонки) откатываются:
     data_rows-строки удаляются, колонки остаются (безопасно);
   - после любой мутации метаданных — `Settings::clear_cache()`.
