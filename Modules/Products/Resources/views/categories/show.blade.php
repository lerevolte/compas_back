                    ...
                    <div class="c-body p-0">
                        <ul class="storages-list c-drag-list list-unstyled mb-0 ui-sortable">
                            ...
                            @foreach($categories as $item)
                                <li>
                                    <div class="d-flex align-items-center position-relative @if($item->id == $category->id || in_array($item->id, $parents)) active @endif">
                                        <a href="{{ route('categories.show', $item->id) }}">
                                            @if($item->children->count())
                                            <button class="treeview-toggler" data-toggle="collapse" data-target=".list-{{ $item->id }}" @if($item->id == $category->id || in_array($item->id, $parents)) aria-expanded="true" @else aria-expanded="false" @endif><i class="fa fa-caret-down"></i></button>
                                            @else
                                    ...
                                    @if($item->children->count())
                                    <ul class="collapse list-{{ $item->id }} @if($item->id == $category->id || in_array($item->id, $parents)) show @endif">
                                        @foreach($item->children as $child)
                                        <li>
                                            <div class="d-flex align-items-center position-relative @if($child->id == $category->id || in_array($child->id, $parents)) active @endif">
                                                
                                                <a class="pl-25" href="{{ route('categories.show', $child->id) }}">
                                                    @if($child->children->count())
                                                    <button class="treeview-toggler" data-toggle="collapse" data-target=".list-{{ $child->id }}" @if($child->id == $category->id || in_array($child->id, $parents)) aria-expanded="true" @else aria-expanded="false" @endif ><i class="fa fa-caret-down"></i></button>
                                                    @else
                                                    <button class="treeview-toggler" ></button>
                                                    @endif
                                                    {{ $child->name }}
                                            ...
                                            @if($child->children->count())
                                            <ul class="collapse list-{{ $child->id }} @if($child->id == $category->id || in_array($child->id, $parents)) show @endif">
                                                @foreach($child->children as $subchild)
                                                <li>
                                                    <div class="position-relative @if($subchild->id == $category->id) active @endif">
