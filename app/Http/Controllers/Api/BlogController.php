...
class BlogController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $list = \App\Models\EntityObject::list('articles', $request);
    ...
    {
        $object = \App\Models\Article::whereJsonContains('slug->value', $slug)->orWhere('slug', $slug)->first();
        if(!$object) {
            return [
                'error' => array(
        ...
        $question_ids = [];
        if($object->detail_text) {
            $detail_blocks = json_decode($object->detail_text, true);
            
            if(is_array($detail_blocks)) {
                foreach($detail_blocks as $block) {
                    if($block['type'] == 'question') {
                        $question_ids[] = $block['questionId'];
                    }
                }
            }
        ...
        if(count($question_ids)) {
            $r = new Request(['ids' => $question_ids]);
            $questions = \App\Models\EntityObject::list('faq', $r);
        }
        $fields_data['questions'] = $questions;
