...
class GuideController extends Controller
{
    ...
    {
        $object = \App\Models\Guide::whereJsonContains('slug->value', $slug)->orWhere('slug', $slug)->first();
        if(!$object) {
            return [
                'error' => array(
    ...
    public function detail_by_id($id, Request $request)
    {
        $object = \App\Models\Guide::where('id', $id)->first();
        if(!$object) {
            return [
                'error' => array(
                    'message' => 'Object not found',
