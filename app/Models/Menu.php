    ...
    public static function get($slug, $settings = null)
    {
        ...
        foreach($menu as $k => $menu_item) {
            ...
            if(isset($menu_item['slug']) && isset($s['models'][$menu_item['slug']]) &&
                ...
                unset($menu[$k]);
            } elseif(in_array($slug, ['logistic_tasks', 'pickups', 'deals', 'supplier_orders', 'payment_invoices', 'expense_invoices', 'product_returns', 'receipt_invoices', 'addresses']) && isset($menu_item['tab']) && $menu_item['tab'] == 'products' && !$user->is_admin
