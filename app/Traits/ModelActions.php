...
trait ModelActions
{
	...
	public static function getFields()
	{
	    ...
	    // if($row_type->id == 39) {
	    // 	$fields = \DB::table('data_rows')->where(
		//     	[
		//     		'data_type_id' => $row_type->id, 
		//     		'account_id' => (Auth::user() && Auth::user()->account_id ? Auth::user()->account_id : 1), 
		//     		'is_remove' => 0,
		//     		['type', '!=', 'text_group']
		//     	])->orWhere([
		//     		'data_type_id' => $row_type->id, 
	...
	public static function getFieldsByRoute(string $route)
	{
	    ...
	    $row_type = \DB::table('data_types')->where('name', $table)->first();
	    $fields = \DB::table('data_rows')->where(['data_type_id' => $row_type->id, 'account_id' => (Auth::user() && Auth::user()->account_id ? Auth::user()->account_id : 1), 'is_remove' => 0])->orderBy('section_id')->orderBy('sort')->get();
	...
	public static function getListData($params)
	{
        ...
        $visible_fields = \App\Models\Field::getVisibleFields($params['slug']);
            ...
            foreach($model_fields as $field) {
                ...
                if(!array_key_exists($field->field, $data) && $field->type != 'text_group') {
                    ...
                    $data[$field->field] = array(
                        ...
                        'can_edit' => 1,//!$settings[$params['slug']]['perms'][$field->field]['write'] ? 1 : 0,
                        'color' => $field_colors[$field->field],
                        'is_plural' => ($field->type == 'text' ? 1 : $field->is_plural)
                    );
                    if(isset($settings[$params['slug']]['list_values'][$field->field])) {
                        $data[$field->field]['options'] = $settings[$params['slug']]['list_values'][$field->field];
                    };
	...
	public function getData($changed_fields = [])
	{
        ...
        foreach($model_fields as $field) {
            ...
            if(!array_key_exists($field->field, $fields_data) && $field->type != 'text_group' && (in_array($field->id, $changed_fields) || !count($changed_fields))) {
                ...
                $fields_data[$field->field] = array(
                    ...
                    'type' => $field->type,
                    'read_only' => $field->only_read,
                    'can_edit' => 1,//!$settings[$slug]['perms'][$field->field]['write'] ? 1 : 0,
                    'color' => $field_colors[$field->field],
                    'is_plural' => ($field->type == 'text' ? 1 : $field->is_plural),
                    'unit' => $field->unit
                );
                ...
                if(isset($settings[$slug]['list_values'][$field->field])) {
                    if($field->type == 'relation') {
                        ...
                        } elseif($this->{$field->field} && isset($settings[$slug]['list_values'][$field->field][$this->{$field->field}])) {
                            $field_values[$this->{$field->field}] = $settings[$slug]['list_values'][$field->field][$this->{$field->field}];
            ...
            if(!array_key_exists($field->field, $fields_data) && (in_array($field->id, $changed_fields) || !count($changed_fields))) {
                ...
                if(!$field->group_id) {
                    $fields_data = array(
                        ...
                        'value' => !is_array($this->{$field->field}) && ValueHelper::isJson($this->{$field->field}) && $field->field != 'products' && is_array(json_decode($this->{$field->field}, true)) ? json_decode($this->{$field->field}, true) : $this->{$field->field},
                        'unit' => $field->unit,
                        'sort' => $field->sort,
                        'mask' => $field->mask,
                        'editableFields' => array(
                            ...
                            'label_color' => ($field->label_color ? $field->label_color : '#000'),
                            'button_name' => $field->button_name,
                    ...
                    if($field->type == 'relation' && $field->is_plural) {
                        $values = $fields_data['value'];
                        $fields_data['value'] = array();
                        if(is_array($values)) {
                            foreach($values as $val) {
                                if(isset($settings[$slug]['list_values'][$field->field][$val]))
                                    $fields_data['value'][] = $settings[$slug]['list_values'][$field->field][$val]['value'];
                            }
                        }
                    } elseif($field->type == 'relation' && $this->{$field->field} && isset($settings[$slug]['list_values'][$field->field][$value])) {
                        $fields_data['value'] = array($settings[$slug]['list_values'][$field->field][$value]['value']);
                    } elseif($field->type == 'relation') {
                        $fields_data['value'] = null;
                    };
                    if(isset($settings[$slug]['list_values'][$field->field])) {
                        $values = array();
                        if($field->type == 'relation') {
                            ...
                            if($field->is_plural && is_array($fields_data['value'])) {
                                foreach($fields_data['value'] as $field_val) {
                                    $field_values[$field_val] = $settings[$slug]['list_values'][$field->field][$field_val];
                                }
                            } elseif($this->{$field->field} && isset($settings[$slug]['list_values'][$field->field][$this->{$field->field}])) {
                                $field_values[$this->{$field->field}] = $settings[$slug]['list_values'][$field->field][$this->{$field->field}];
                        ...
                        if($field->type == 'status') {
                            ...
                            foreach($settings[$slug]['list_values'][$field->field] as $option) {
                            ...
                            foreach($settings[$slug]['list_values'][$field->field] as $k => $option) {
                        ...
                        if($field->type == 'status')
                            $fields_data['editableFields']['statuses'] = $settings[$slug]['list_values'][$field->field];
                    };
                    ...
                    if($field->type == 'text_group') {
                        ...
                        foreach($subfields as $subfield) {
                            $fields_data['options'][$subfield->id] = $subfield->display_name;
                            $fields_data['options_key_value'][$subfield->id] = $subfield->display_name;
                            $values[] = array(
                                'id' => $subfield->id,
        ...
        } elseif(isset($this->display_name)) {
            $list_val = array(
                'value' => $this->id,
    ...
    public function saveRelations($field_name, $value)
    {
        ...
        if(isset($settings[$slug]['list_values'][$field_name])) {

            $field = $settings[$slug]['fields'][$field_name];
            $list_values = $settings[$slug]['list_values'][$field_name];
            ...
            if($field->is_plural) {
                ...
                $arr = json_decode($this->{$field_name}, true);
                if(is_array($arr)) {
                    $old_list = $arr;
                ...
                $old_value = isset($list_values[$this->{$field_name}]) ? $list_values[$this->{$field_name}] : '';
                $new_value = isset($list_values[$value]) ? $list_values[$value] : '';
                $history_text = $field->display_name.': '.(is_array($old_value) ? $old_value['text'] : $old_value).' -> '.(is_array($new_value) ? $new_value['text'] : $new_value);
            }
            if($history_text) {
                if(isset(\Auth::user()->id))
                    $history = new \App\Models\History(['entity' => $slug, 'entity_id' => $this->id, 'user_id' => \Auth::user()->id, 'text' => $history_text]);
                else
                    $history = new \App\Models\History(['entity' => $slug, 'entity_id' => $this->id, 'user_id' => 0, 'text' => $history_text]);
                $history->save();
            ...
            if(array_key_exists($field->field, $row) && (array_key_exists($field->field, $object) && $object[$field->field] !== $row[$field->field] || !array_key_exists($field->field, $object))) {
                ...
                if($field->type == 'status' && isset($settings[$slug]['list_values'][$field->field])) {
                    ...
                    if($old_status && $new_status) {
                        ...
                        $history_text = $field->display_name.': '.(isset($object[$field->field]) ? $object[$field->field]:'').' -> '.$new_value;
                    } else {
                        $history_text = $field->display_name.': '.(isset($object[$field->field]) ? $object[$field->field]:'').' -> '.$row[$field->field];
                    }
