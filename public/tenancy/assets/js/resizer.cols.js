    ...
    "_fnStateSave": function ( oState )
    {
        ...
        if ( oState.aaSorting ) {
            ...
            for ( i=0 ; i<oState.order.length ; i++ ) {
                if(columns[ oState.order[i][0] ] != undefined)
        ...
        if(this.dom.resize) {
            ...
            if(!scrollXEnabled && !scrollYEnabled) {
                ...
                if(moveLength < 0) {
                    ...
                    //console.log('MINW '+minNThWidth)
                    if(nThWidth >= minNThWidth) {
                        $nTh.width(nThWidth);
                        //console.log('W1 '+nThWidth);
                    }
                    else {
                        $nTh.width(minNThWidth);
                        //console.log('W2 '+minNThWidth);
                    }
                }
                else {
                    if($nTh.width() - 1 > 1000) {
                    ...
                    //console.log('W3 '+this.s.mouse.nextStartWidth);
                    if(nThNextInnerWidth <= minNThNextWidth) { return; }

                    $nTh.width(nThWidth);
                    //console.log('W4 '+nThWidth);
                }
                $nTh.closest('table').find('.input-table-fix:checked').each(function(index, el) {
                    $nTh.closest('table').find('div[data-field="'+$(this).val()+'"]').closest('td').addClass('sticky-td');
                });
                var th_width = 0;
                $nTh.closest('table').find('th.sticky-head').each(function(i, elem){
                    $(this).css({'left': th_width+'px'});
                    var indx = $(this).index();
                    $nTh.closest('table').find('tr').each(function(j, tr){
