        ...
        @foreach($photos as $photo)
            ...
            <div class="file-item file-item-pdf" data-id="{{ $photo->id }}">
                <div class="file-control-wrap">
                    ...
                    <ul class="dropdown-menu dropdown-menu__actions" aria-labelledby="dd184" x-placement="bottom-start" style="position: absolute; transform: translate3d(200px, 26px, 0px); top: 0px; left: 0px; will-change: transform;">
                        <li><a href="{{ '/storage/tenant'.$tenant.'/app/'.$photo->path}}" target="_blank" class="dropdown-item">Просмотреть</a></li>
                        <li><span class="dropdown-item js-delete-file" data-id="{{ $photo->id }}"><span class="text-danger">Удалить</span></span></li>
                    </ul>
                </div>
                <a href="{{ '/storage/tenant'.$tenant.'/app/'.$photo->path}}" data-fancybox="gallery-{{ $field->id }}" target="_blank">
            ...
            <div class="file-item" data-id="{{ $photo->id }}">
                <div class="file-control-wrap">
                    ...
                    <ul class="dropdown-menu dropdown-menu__actions" aria-labelledby="dd184" x-placement="bottom-start" style="position: absolute; transform: translate3d(200px, 26px, 0px); top: 0px; left: 0px; will-change: transform;">
                        <li><a href="{{ str_replace('/public', '/tenant'.$tenant.'/app/public', Storage::disk('public')->url($photo->path)) }}" data-fancybox="{{ $field->field }}" class="dropdown-item">Просмотреть</a></li>
                        <li><span class="dropdown-item js-delete-file" data-id="{{ $photo->id }}"><span class="text-danger">Удалить</span></span></li>
                    </ul>
                </div>
                <a href="{{ '/storage/tenant'.$tenant.'/app/'.$photo->path }}" data-fancybox="gallery-{{ $field->id }}">
                    <img src="{{ \Thumbnail::src(str_replace('/public', '/tenant'.$tenant.'/app/public', Storage::disk('public')->url($photo->path)))->heighten(200)->url() }}" height="96">
                </a>
            </div>
