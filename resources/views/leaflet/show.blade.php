...
<body>
    ...
    <script>
        ...
        async function showMultipleRoutes(routesData) {
            ...
            const routePromises = routesData.map(routeData => {
                return new Promise((resolve) => {
                    ...
                    const waypoints = tasksWithAddress.map(task => {
                    ...
                    const control = L.Routing.control({
                        ...
                        createMarker: () => null,
                        lineOptions: {
                            styles: [{ color: '#8601ff', opacity: 0.7, weight: 5 }]
                        }
                    }).addTo(map);
        ...
        routesFromLaravel.forEach((route, i) => {
            console.log(`Route ${i}:`, route.name);
            console.log('Tasks count:', route.tasks?.length);
            route.tasks?.forEach((task, j) => {
                console.log(`  Task ${j}:`, task.address);
                try {
                    const addr = JSON.parse(task.address);
                    console.log('    Parsed coords:', addr.coords);
                } catch(e) {
