...
@section('title')
Удаленные {{ $entity->display_name_plural }}
@endsection
@section('h1')
Удаленные {{ $entity->display_name_plural }}
@endsection
    ...
    <div class="header__top">
      <div class="header__left">
        ...
        <div class="header-breadcrumbs">
          ...
          <a class="header-breadcrumbs__link @if($active_menu->id == $item->id) header-breadcrumbs__link_active @endif" href="/trash/{{ $item->slug }}">{{ $item->display_name_singular }}</a>
          @endforeach
        </div>
      </div>
    </div>
...
@section('content')
                  ...
                  <div class="table-options__inner">
                    <div class="dropdown" data-dropdown>
                      ...
                      <div class="dropdown__menu dropdown__menu_align dropdown__menu_sm-lh" data-dropdown="menu">
                        <ul class="dropdown__list">
                          <li class="dropdown__item">
                            <button class="dropdown__link dropdown__link_red js-restore-model" data-id="{{ $object['id'] }}" data-model="{{ $model }}" type="button" >Восстановить</button>
                          </li>
                        </ul>
                      </div>
                    </div>
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      $('body').on('click', '.js-restore-model', function(){
        var id = $(this).data('id'),
            tr = $(this).closest('tr');

        $.ajax({
            type: 'post',
            url: '/objects/{{ $model }}/restore/'+id,
            async: false,
            data: {
